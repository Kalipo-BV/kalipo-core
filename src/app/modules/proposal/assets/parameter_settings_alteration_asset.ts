...
import { ProposalType } from '../../../database/enums';
...
export class ParameterSettingsAlterationAsset extends BaseAsset {
	...
	public async apply({ asset, transaction, stateStore }: ApplyAssetContext<{}>): Promise<void> {
		...
		if (proposalTypeFound) {
			...
			let typeFound = false;
			const availableProposalTypes = Object.values(ProposalType);
			for (let index = 0; index < availableProposalTypes.length; index++) {
				const type = availableProposalTypes[index];
				if (type == input.proposalType) {
					const newProvision: ProposalProvisionsI = {
						type: type,
						attendance: input.mandatoryAttendence,
						acceptance: input.acceptence,
